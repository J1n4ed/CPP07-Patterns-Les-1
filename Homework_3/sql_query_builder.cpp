...
jinx::AdvancedSqlSelectQueryBuilder & jinx::AdvancedSqlSelectQueryBuilder::AddWhere(std::string lval, std::string rval, std::string oper) noexcept
{
    if (!whereCalledOnce)
    {
        QWhere = lval + oper + rval;
    }
    else
    {
        QWhere = QWhere + " AND " + lval + oper + rval;
    }
